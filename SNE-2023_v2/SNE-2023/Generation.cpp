	...
	void GenerateFunction(int indexInLexTable)
	{
		...
		for (int i = indexInLexTable; i > 0; i++)
		{
			...
			case LEX_EQUAL:
			{
				...
				bool isNumb;  //true numb    false text
				if (idtable.table[lextable.table[i - 1].idxTI].iddatatype == IT::LIT)
					isNumb = true;
				else 
					isNumb = false;
				...
				while (lextable.table[i].lexema != LEX_SEMICOLON && 
					lextable.table[i].lexema != LATTICE)
				{
					isID = false;
					switch (lextable.table[i].lexema)
					{
					...
					case LEX_LITERAL:
					{
						if (lextable.table[i].value == LEX_LIBFUNCTION) {
							...
							case -2:
								...
								break;
							case -3:
								buffstr += "\tpush texttolit\n";
								MainStack.push("texttolit");
							default:
								break;
							}
							break;
			...
			case LEX_OUTPUT:
			{
				...
				if (idtable.table[lextable.table[i + 2].idxTI].iddatatype == IT::LIT) {
					if ((idtable.table[lextable.table[i + 2].idxTI].idtype == IT::V) ||
						(idtable.table[lextable.table[i + 2].idxTI].idtype == IT::P))
						isID = true;
