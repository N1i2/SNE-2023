...
namespace MFST
{
	...
	Mfst::Mfst(LexA::Tables plex, GRB::Greibach pgrebach)
	{
		grebach = pgrebach;
		lex = plex;
		lenta = new short[lenta_size = lex.mylextable.size];
		for (int k = 0; k < lenta_size; k++) lenta[k] = TS(lex.mylextable.table[k].lexema);
		lenta_position = 0;
	...
	Mfst::RC_STEP Mfst::step(std::ofstream& filetrace)
	{
		RC_STEP rc = SURPRISE;
			...
			if (ISNS(st.top()))
			{
				...
				if ((nrule = grebach.getRule(st.top(), rule)) >= 0)
				{
					...
					else
					{
						...
						rc = reststate(filetrace) ? NS_NORULECHAIN : NS_NORULE;
					};
				}
				else rc = NS_ERROR;
			}
