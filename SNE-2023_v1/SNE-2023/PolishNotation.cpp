...
{
	bool bufferb;
	for (int i = 0; i < tables.mylextable.size; i++) 
	{
		if (tables.mylextable.table[i].lexema == LEX_EQUAL &&
			tables.mylextable.table[i - 3].lexema != LEX_CHECK)
		{
			i++;
...
bool polishNotation(int lextable_pos, LT::LexTable& lextable, IT::IdTable& idtable)
{
	...
	{
		countOfAllInputLex++;
		if (lextable.table[lextable_pos].lexema != LEX_LEFTTHESIS &&
			lextable.table[lextable_pos].lexema != LEX_RIGHTTHESIS)
			counter++;
		if (lextable.table[lextable_pos].lexema == LEX_COMMA &&
			lextable.table[lextable_pos - 2].lexema != LEX_LEFTTHESIS)
			counter++;
		lextable_pos++;
	...
	for (int i = 0; (i < countOfAllInputLex + 1) || lextable.table[startLex + i - 1].lexema == LEX_SEMICOLON; i++)
	{
		...
		case LEX_OPERATOR:
			...
			if (lextable.table[lextable_pos + i].value == '*' ||
				lextable.table[lextable_pos + i].value == '/' || 
				lextable.table[lextable_pos + i].value == '%')
		...
		default:
			if (isAFunction)
			{
				...
				{
					if (lextable.table[lextable_pos + i].lexema == LEX_LITERAL ||
						lextable.table[lextable_pos + i].lexema == LEX_ID)
					{
						countOfParameters++;
						AlmostAllEntries[counter1] = lextable.table[lextable_pos + i];
						AlmostAllEntries[counter1].index = buffershort;
