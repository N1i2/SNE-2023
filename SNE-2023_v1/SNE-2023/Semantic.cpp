...
namespace Semantic
{
		...
		for (int i = 0; i < myTables.mylextable.size; i++) {
			...
			case LEX_EQUAL:
			{
				...
				while (myTables.mylextable.table[++j].lexema != LEX_SEMICOLON &&
					myTables.mylextable.table[j].lexema != LATTICE)
				{
	...
	void CheckSemanticBeforePolish(LexA::Tables myTables)
	{
		...
		if (IT::IsId(myTables.myidtable, (unsigned char*)mainFunctionStrName.c_str()) == TI_NULLIDX)
			throw ERROR_THROW(151);

		for (int i = 0; i < myTables.mylextable.size; i++)
		{
			if (myTables.mylextable.table[i].lexema == LEX_DIRSLASH &&
				myTables.mylextable.table[i].value == '/')
			{
			...
			if (myTables.mylextable.table[i].lexema == LEX_EQUAL &&
				myTables.mylextable.table[i - 3].lexema != LEX_CHECK)
			{
				int k = i;
				int counter = 0;
				...
				while (myTables.mylextable.table[k++].lexema != LEX_SEMICOLON)
				{
					if (myTables.mylextable.table[k].value == LEX_LIBFUNCTION)
					{
						...
						if (myTables.mylextable.table[k].idxTI == -3)
						{
					...
					if (myTables.mylextable.table[k].lexema == LEX_RIGHTTHESIS)
						isCheckLibFunction = false;

					if ((myTables.mylextable.table[k].lexema == LEX_ID ||
						myTables.mylextable.table[k].lexema == LEX_LITERAL) &&
						myTables.mylextable.table[k].value != LEX_LIBFUNCTION)
					{
			...
			if (myTables.mylextable.table[i].idxTI != LT_TI_NULLIDX)
			{
					...
					myTables.mylextable.table[i].lexema != LEX_START)
				{
					...
					while (myTables.mylextable.table[++k].lexema != LEX_RIGHTTHESIS)
					{
						if (myTables.mylextable.table[k].lexema == LEX_LITERAL ||
							myTables.mylextable.table[k].lexema == LEX_ID)
						{
					...
					k = myTables.myidtable.table[myTables.mylextable.table[i].idxTI].idxfirstLE;
